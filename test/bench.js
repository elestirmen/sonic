...
//   node test/bench.js [--trials 20] [--profile normal,hizli]
...
import { decodeSamples, messages, text } from './helpers.js';
...
for (const cond of CONDITIONS) {
  ...
  for (const p of profiles) {
    ...
    for (let t = 0; t < trials; t++) {
      ...
      const { samples } = encodeMessage(new TextEncoder().encode(msg), p.key, fsTx);
